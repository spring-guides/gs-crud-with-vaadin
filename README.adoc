...
Additionally, you might want to override the used Vaadin version into the newest available version. This can be easily done by defining the Vaadin Bill of Materials (BOM) like this:
...
Gradle doesn't support "BOMs" by default, but there is a handy https://plugins.gradle.org/plugin/io.spring.dependency-management[plugin for that]. Check out the build.grade file for an example on how to use it.
