...
Data JPA and H2 dependencies. (You will add the Vaadin dependency later.) The following
...
can also use Gradle. It also shows values of `com.example` and `crud-with-vaadin` as the
...
This guide is a continuation from link:/gs/accessing-data-jpa[Accessing Data with JPA].
